...
def mock_connection(monkeypatch, dummy_contacts):
    ...
    phone_numbers = [(c.db_id, c.phone_personal, c.phone_work, c.phone_home)
                     for c in dummy_contacts]
    contact_info = [(c.db_id, c.first_name, c.last_name, c.email, c.address, c.date_added)
                    for c in dummy_contacts]
    ...
    conn.commit()
    monkeypatch.setattr(cbook.datamanager.sqlite3,
                        'connect', lambda *args, **kwargs: conn)
...
def test_create_contact():
    ...
    contact = Contact(init_count + 1, "Rahul", "Verma",
                      datetime.datetime.now(), "+321894123572")
    assert mgr.create_contact(contact)
    assert mgr.get_contact_count() == init_count + 1
    contact = Contact(init_count + 2, "Kunal", "Verma",
                      datetime.datetime.now(), "+321894123372", "+321894123572")
    assert mgr.create_contact(contact)
    assert mgr.get_contact_count() == init_count + 2


@pytest.mark.skip(reason="Problem with schema")
def test_update_contact():
    mgr = DataManager()
    contact = Contact(db_id=20, first_name='Prakash', last_name='Gupta', date_added=datetime.datetime(2022, 10, 23, 14, 44, 19, 768495),
                      phone_personal='+688532624571', phone_work='+648202864525', phone_home='+720360101603', email="parkash@gmail.com", address=None)
    assert mgr.update_contact(contact)

    # Failing phone no. length constraint
    contact = Contact(db_id=20, first_name='Prakash', last_name='Gupta', date_added=datetime.datetime(2022, 10, 23, 14, 44, 19, 768495),
                      phone_personal='+6885326dfsaff24571', phone_work='+648202864525', phone_home='+720360101603', email=None, address=None)
    assert not mgr.update_contact(contact)
